...
#     def forward(self, x):
#         # Input for LSTM needs to be shaped as (batch_size, seq_length, features)
#         x = x.view(x.size(0), 1, -1) 

#         # Process through LSTM layer; output and hidden state are returned
#         lstm_out, self.hidden = self.lstm(x)  

#         # Take the output of the last time step
#         out = self.seq(lstm_out[:, -1, :]) 
#         return out

#     def init_hidden(self, batch_size):  
#         # Initializes hidden and cell states of the LSTM (zeroed)
